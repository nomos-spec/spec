...
class ExprParser {
  ...
  private comparison(): unknown {
    ...
    if (t && t.type === "op" && ["==", "!=", ">", ">=", "<", "<="].includes(t.value)) {
      this.next(); const right = this.arith();
      switch (t.value) {
    ...
    if (this.isKeyword("in")) { this.next(); const right = this.arith(); return Array.isArray(right) && right.some(v => jsEq(v, left)); }
    return left;
  }

  private arith(): unknown {
    let left = this.term();
    while (this.peek()?.type === "op" && ["+", "-"].includes(this.peek()!.value)) {
      const op = this.next().value; const right = this.term();
  ...
  private term(): unknown {
    ...
    while (this.peek()?.type === "op" && ["*", "/"].includes(this.peek()!.value)) {
      const op = this.next().value; const right = this.factor();
  ...
  private callFunction(name: string, args: unknown[]): unknown {
    switch (name) {
      ...
      case "len":        return Array.isArray(args[0]) || typeof args[0] === "string" ? (args[0] as string | unknown[]).length : 0;
      ...
      // Per §4.2: a runtime MUST NOT fail silently on an unrecognised operator/function —
      // this is the real analog of the old AST's "unsupported operator", tested by v08.
